...
- Staging `bundleAssetHost` is unused for MatIcon SVGs (icons are inlined via `addSvgIconLiteral`); keep empty so any remaining absolute asset URLs stay same-origin.
