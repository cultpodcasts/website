...
describe('authInterceptor', () => {
    ...
    function setup(platformId: object = 'browser' as unknown as object) {
        getAccessTokenSilently = vi.fn().mockName('getAccessTokenSilently').mockReturnValue(of('test-token'));
    ...
    it('propagates API error responses without retrying unauthenticated', () => {
        ...
        expect(req.request.headers.get('Authorization')).toBe('Bearer test-token');
        req.flush({ tweeted: false, failedTweetContent: 'hello tweet' }, { status: 400, statusText: 'Bad Request' });
